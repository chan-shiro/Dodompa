#!/usr/bin/env node
/**
 * Dodompa Production MCP Server
 *
 * High-level task management tools for Claude Desktop / Claude Code.
 * Proxies to the Electron app's HTTP server (port 19876).
 *
 ...
 *     "dodompa": {
 *       "command": "npx",
 *       "args": ["-y", "tsx", "/path/to/Dodompa/mcp/src/production.ts"]
 *     }
 *   }
 * }
 */
...
const server = new McpServer({
  name: 'dodompa',
  version: '1.0.0',
  description: 'Dodompa — AI-powered RPA task manager',
}, {
  instructions: `Dodompa は「AIのためのRPA」です。ユーザーがあらかじめ作成したタスク（Webブラウザ・macOSアプリの自動化）を実行できます。
...
- Dodompaアプリが起動していない場合、各ツールが明示的にエラーを返す。ユーザーにアプリを起動するよう伝える。`,
})
...
/**
 * Verify Dodompa app is running. Throws a clear, user-actionable error if not.
 * Use at the top of every tool that depends on Dodompa being up.
 */
async function ensureDodompaRunning(): Promise<void> {
  let resp: Response | null = null
  try {
    resp = await fetch(`${DODOMPA_URL}/health`, { signal: AbortSignal.timeout(2000) })
  } catch (e) {
    throw new Error(
      `Dodompa アプリが起動していません (${DODOMPA_URL}/health に接続できません)。\n` +
      `アプリを起動してから再実行してください。\n` +
      `起動方法: cd /path/to/Dodompa && pnpm dev\n` +
      `内部エラー: ${(e as Error).message}`
    )
  }
  if (!resp.ok) {
    throw new Error(`Dodompa アプリのヘルスチェックが失敗しました: HTTP ${resp.status}`)
  }
}
...
async function ipc(channel: string, ...args: unknown[]): Promise<unknown> {
  let resp: Response
  try {
    resp = await fetch(`${DODOMPA_URL}/ipc/${channel}`, {
      method: 'POST',
    ...
    throw new Error(
      `Dodompa アプリに接続できません (${channel})。アプリが起動しているか確認してください: ${(e as Error).message}`
    )
  }
  if (!resp.ok) {
    const text = await resp.text()
    throw new Error(`Dodompa error (${channel}): ${resp.status} ${text}`)
  }
  const data = await resp.json() as { result?: unknown }
  return data.result
}

async function emit(channel: string, params: unknown): Promise<void> {
  let resp: Response
  try {
    resp = await fetch(`${DODOMPA_URL}/emit/${channel}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  if (!resp.ok) {
    const text = await resp.text()
    throw new Error(`Dodompa emit error (${channel}): ${resp.status} ${text}`)
  }
}
...
async function dbQuery(sql: string): Promise<unknown[]> {
  let resp: Response
  try {
    resp = await fetch(`${DODOMPA_URL}/db/query?sql=${encodeURIComponent(sql)}`)
  } catch (e) {
    throw new Error(
      `Dodompa アプリに接続できません (DB query)。アプリが起動しているか確認してください: ${(e as Error).message}`
    )
  }
  if (!resp.ok) throw new Error(`DB query error: ${resp.status}`)
...
server.tool(
  ...
  {},
  async () => {
    const tasks = await ipc('task:list') as Array<{
    ...
    const summary = tasks.map(t => ({
      ...
      // What the task does (prefer initial instruction since it's stable)
      instruction: t.initialInstruction || t.instruction || undefined,
      // What the task produces / its purpose
      goal: t.goal || undefined,
      stepCount: Array.isArray(t.steps) ? t.steps.length : 0,
      // Variables the task takes as input
...
server.tool(
  ...
  async ({ name }) => {
    const task = await ipc('task:create', name)
    return json(task)
  }
)
...
server.tool(
  ...
  {
    taskId: z.string().describe('タスクID'),
    data: z.record(z.unknown()).describe('更新するフィールド（例: {"instruction": "...", "goal": "..."}）'),
  },
  async ({ taskId, data }) => {
    const task = await ipc('task:update', taskId, data)
    return json(task)
  }
)
...
server.tool(
  ...
  async ({ taskId, instruction }) => {
    await ensureDodompaRunning()
    const startedAt = new Date().toISOString()
    await emit('ai:startAutonomousGeneration', { taskId, instruction })

    // 起動後 (1 秒) で generation_step_logs に新規エントリが入っているか確認
...
server.tool(
  ...
  async ({ taskId }) => {
    await ensureDodompaRunning()
    const questions = await ipc('ai:listPendingQuestions', taskId ?? undefined) as Array<{
...
server.tool(
  ...
  async ({ questionId, answer }) => {
    await ensureDodompaRunning()
    const result = await ipc('ai:answerQuestion', questionId, answer) as { ok: boolean; error?: string; taskId?: string }
    if (!result.ok) {
...
server.tool(
  ...
  async ({ taskId, since, timeoutSec, pollIntervalMs }) => {
    ...
    while (Date.now() < deadline) {
      // 1) pending question check (止まっている場合ユーザー入力待ちの可能性)
      const pending = await ipc('ai:listPendingQuestions', taskId) as Array<{
        id: string; taskId: string; text: string; infoKey?: string; askedAt: string
      }>
      if (pending.length > 0) {
        return json({
      ...
      if (logs.length > 0) {
        ...
        if (lastPhase === 'done') {
          // Return recent logs for context
...
server.tool(
  ...
  { taskId: z.string().describe('タスクID') },
  async ({ taskId }) => {
    await ipc('ai:cancelGeneration', taskId)
...
server.tool(
  ...
  async ({ taskId, since, limit }) => {
    ...
    // Check for pending user questions — these block the pipeline.
    const pendingQuestions = await ipc('ai:listPendingQuestions', taskId) as Array<{
      id: string; taskId: string; text: string; infoKey?: string; askedAt: string
    }>
...
// ─── Task Refactor (async: start → poll) ───
...
server.tool(
  ...
  async ({ taskId, instruction }) => {
    const result = await ipc('ai:refactorTask', { taskId, instruction, referenceTaskIds: [] })
    return json(result)
  }
)
...
server.tool(
  ...
  {
    taskId: z.string().describe('タスクID'),
    variables: z.record(z.string()).optional().default({}).describe('実行変数(例: {"companyName": "株式会社xxx"})'),
  },
  async ({ taskId, variables }) => {
    // 1) Dodompa アプリ起動チェック (繋がらなければ即座にエラー)
    await ensureDodompaRunning()

    // 2) タスクが存在することを確認 (存在しない ID で実行リクエストすると失敗する)
    let taskName = ''
    try {
      const task = await ipc('task:get', taskId) as { id?: string; name?: string; steps?: unknown[] } | null
      if (!task || !task.id) {
    ...
    // 3) 実行開始 — 起動初期 (約1秒) で同期エラーが出ないか同時に監視
    //    runner:execute 全体は長時間かかるので await しないが、
    //    起動直後の早期エラー(IPC ハンドラ未登録、起動例外等)は捕まえる
    const startedAt = new Date().toISOString()
    const execPromise = ipc('runner:execute', taskId, variables)
    ...
    // execPromise の長期 reject を握りつぶさないための安全網
    execPromise.catch(e => {
      console.error(`[task_run] runner:execute eventually failed for ${taskId}:`, e)
    })
    ...
    // 4) 起動後 (1.2 秒経過) — 実行ログに新しいエントリが入っているはず。確認。
...
server.tool(
  ...
  async ({ taskId, limit }) => {
    const result = await ipc('log:listExecutions', taskId ?? undefined)
...
server.tool(
  ...
  async () => {
    try {
      const resp = await fetch(`${DODOMPA_URL}/health`)
      const data = await resp.json()
      return json({ connected: true, ...data as Record<string, unknown> })
    } catch (e) {
      return text(`Dodompaアプリに接続できません: ${(e as Error).message}\nアプリを起動してください。`)
    }
  }
)
